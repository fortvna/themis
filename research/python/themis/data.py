...
def load_from_spec(spec: dict[str, Any], *, root: Path | None = None, network: bool = True, csv_path: str | None = None) -> SeriesLoad:
    spec = dict(spec)
    data_block = dict(spec.get("data") or {})
    if csv_path:
        data_block["source"] = "csv"
        data_block["csv_path"] = str(csv_path)
        spec["data"] = data_block
    source = (data_block.get("source") or "csv").lower()
    inst = spec.get("instrument") or {}
    symbol = inst.get("symbol") or data_block.get("symbol") or ""
    timeframe = inst.get("timeframe") or data_block.get("timeframe") or "4h"
    root = root or research_root()
    if source == "vision" and not network:
        cached = cache_path(symbol, timeframe, root)
        if cached.exists():
            data_block = dict(data_block)
            data_block["source"] = "csv"
            data_block["csv_path"] = str(cached)
            spec["data"] = data_block
            source = "csv"
        else:
            raise DataError("vision cache empty and network disabled")
    df, meta = load_bars(spec, root=root)
    ident = {
        "provider": meta.get("provider") or "binance",
        "symbol": symbol,
        "exchange": meta.get("exchange") or "binanceusdm",
        "timeframe": timeframe,
    ...
    return SeriesLoad(
        df=df,
        provider=str(meta.get("provider") or "binance"),
        symbol=str(symbol),
        timeframe=str(timeframe),
        exchange=str(meta.get("exchange") or "binanceusdm"),
        source=str(meta.get("source") or source),
        actual_start=meta.get("actual_start"),
        actual_end=meta.get("actual_end"),
        n_bars=int(meta.get("n_bars") or len(df)),
        cache_path=meta.get("csv_path") or meta.get("cache"),
