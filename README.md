...
| Data | **Providers: Binance, Bybit, Bitget.** Pulled with **ccxt**. You name the venue (and usually the symbol). Gold 4h on a named venue → that venue’s `XAUUSDT` perp. |
...
Ask once only for things pandas cannot know: which **provider** (Binance / Bybit / Bitget) if you omitted it, whether costs are zero on purpose, or whether you meant a different timeframe. “Gold on Binance” is Binance `XAUUSDT`. The same English on Bybit or Bitget is **that** venue’s `XAUUSDT` — a different series.
...
**Providers: Binance, Bybit, Bitget.** That is who produced the bars. **ccxt** is only the puller. You name the venue when you ask; optionally the symbol.

Do not build a warehouse. Do not backfill every venue in advance. Do not merge the same ticker across venues.

1. Name **provider** in the ask (`binance` / `bybit` / `bitget`). If you also name the symbol, that is the series. If you say “gold” and a venue but no ticker, that venue’s `XAUUSDT` perp. If you omit the venue, the agent asks once — it does not pick one.
2. Pull with **ccxt** into `research/.cache/<provider>/<exchange>/<symbol>/<tf>.csv` (gitignored). CSV is allowed if you already have the file.
   - Binance: `binance` (spot) or `binanceusdm` (USD-M, including TradFi perps such as `XAUUSDT`)
   - Bybit: `bybit` spot or linear (USDT perps, stock/ETF/commodity tags)
   - Bitget: `bitget` spot or USDT-M (including RWA-tagged perps)
...
4. A lookalike is still that venue’s product. Binance `XAUUSDT` ≠ Bybit `XAUUSDT` ≠ Bitget `XAUUSDT` ≠ `XAUT` ≠ `PAXG` ≠ COMEX. Do not relabel.
5. Forex spot and cash/futures indices are **not** these three books (except a venue’s own named perp/CFD, which must stay labeled). Do not fetch Yahoo/`EURUSD=X` / `NQ=F` through these providers.
...
6. Name the exact instrument. Provider is **binance**, **bybit**, or **bitget** as you said. Same ticker on two venues is two series.
...
| Stage | Tool |
| --- | --- |
| English → YAML | Any terminal agent or a Colab cell. Compile only. Skill file later. |
| Bars | **Binance / Bybit / Bitget** via `ccxt` (or CSV) |
...
| Where | Colab or local Python 3.11+ |
